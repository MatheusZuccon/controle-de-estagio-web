  ...
  template: `
    <main class="detail-page" *ngIf="tce() as item">
      ...
      <section class="card">
        ...
        <div class="content" *ngIf="tab() === 'identification'"><dl><div><dt>Matrícula</dt><dd>{{ item.studentEnrollment }}</dd></div><div><dt>Nome</dt><dd>{{ item.studentName }}</dd></div><div><dt>E-mail</dt><dd>{{ item.studentEmail }}</dd></div><div><dt>Telefone</dt><dd>{{ item.studentPhone }}</dd></div><div><dt>Empresa concedente</dt><dd>{{ item.company.name }}</dd></div><div><dt>Período</dt><dd>{{ item.startDate | date:'dd/MM/yyyy' }} a {{ item.endDate | date:'dd/MM/yyyy' }}</dd></div></dl>
