...
@Component({
  ...
  template: `
    <main class="detail-page" *ngIf="tce() as item">
      ...
      <section class="heading"><div><p class="eyebrow">CONTROLE DE DOCUMENTOS</p><h1>{{ reviewMode ? 'Analisar' : 'Visualizar' }} TCE</h1><p>Nº {{ item.number }} <span>•</span> {{ item.studentName }} <span>•</span> {{ item.company.name }}</p></div><div class="heading-actions"><button type="button" class="secondary header-back" (click)="router.navigateByUrl('/tces')">Voltar</button><span class="status" [class]="'status '+item.status">{{ label(item.status) }}</span></div></section>
      ...
      <section class="card">
        ...
        <div class="content" *ngIf="tab() === 'identification'"><dl><div><dt>Matrícula</dt><dd>{{ item.studentEnrollment }}</dd></div><div><dt>Nome</dt><dd>{{ item.studentName }}</dd></div><div><dt>E-mail</dt><dd>{{ item.studentEmail }}</dd></div><div><dt>Telefone</dt><dd>{{ item.studentPhone }}</dd></div><div><dt>Empresa concedente</dt><dd>{{ item.company.name }}</dd></div><div><dt>Tipo de estágio</dt><dd>{{ item.internshipType === 'SUPERVISIONADO' ? 'Supervisionado' : item.internshipType === 'VOLUNTARIO' ? 'Voluntário' : 'Outro' }}</dd></div><div><dt>Estágio remunerado</dt><dd>{{ item.isPaid ? 'Sim' : 'Não' }}</dd></div><div><dt>Período</dt><dd>{{ item.startDate | date:'dd/MM/yyyy' }} a {{ item.endDate | date:'dd/MM/yyyy' }}</dd></div></dl>
...
export class TceDetailComponent implements OnInit, HasUnsavedChanges {
  ...
  id = ''; reviewMode = false; decision = 'APPROVE'; reason = ''; reviewSubmitted = false;
  ...
  hasUnsavedChanges() { const item = this.tce(); return Boolean(item && this.reviewMode && !this.reviewSubmitted && this.canReview(item) && (this.decision !== 'APPROVE' || this.reason !== '')); }
  ...
  applyReview() { if (this.decision !== 'APPROVE' && !this.reason.trim()) { this.notice('Informe o motivo para solicitar correção ou indeferir.', true); return; } const reason = this.decision === 'APPROVE' ? undefined : this.reason.trim(); this.http.post(`${this.auth.api}/tces/${this.id}/review`, { decision: this.decision, reason }).subscribe({ next: () => { this.reviewSubmitted = true; this.router.navigateByUrl('/tces'); }, error: response => this.notice(response.error?.message || 'Não foi possível registrar a decisão.', true) }); }
